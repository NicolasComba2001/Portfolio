...
const validations =(form)=>{
    ...
    if(!form.mensaje.trim())errores.mensaje = "el campo 'mensaje' es requerido";
...
export default function Formulario(){
    ...
    return(
        ...
        <form className='form' id='conoceMas' onSubmit={handleSubmit} data-aos="fade-right"data-aos-duration="1000">
            ...
            {!error.nombre && <FontAwesomeIcon style={{position: "absolute", top:"0", right:"-25%", margin:"5px 70px"}} icon={["fa","check"]}></FontAwesomeIcon>}<input   className={`formGrupo__info ${error.nombre && "error"}`} type="text" id='nombre'  name='nombre'  onBlur={handleBlur} onChange={handleChange} value={form.nombre} required/> 
                <label   className='formGrupo__titulo' htmlFor='nombre'>Nombre</label>
                 {error.nombre && <p className='FormError'>{error.nombre}</p>}
            </div>
            <div className='formGrupo'>
                {!error.apellido && <FontAwesomeIcon style={{position: "absolute", top:"0", right:"-25%", margin:"5px 70px"}} icon={["fa","check"]}></FontAwesomeIcon>}<input   className={`formGrupo__info ${error.apellido && "error"}`} type="text" id='apellido' name='apellido' onBlur={handleBlur} onChange={handleChange} value={form.apellido} required />
                <label   className='formGrupo__titulo' htmlFor=''>Apellido</label>
                 {error.apellido && <p className='FormError'>{error.apellido}</p>}
            </div>
            <div className='formGrupo'>
                {!error.email && <FontAwesomeIcon style={{position: "absolute", top:"0", right:"-25%", margin:"5px 70px"}} icon={["fa","check"]}></FontAwesomeIcon>}<input   className={`formGrupo__info ${error.email && "error"}`} type="email" id='email'  name='email' onBlur={handleBlur} onChange={handleChange} value={form.email} required/>
                <label   className='formGrupo__titulo' htmlFor='email'>E-mail</label>
                 {error.email && <p className='FormError'>{error.email}</p>}
            </div>
            <div className='formGrupo'>
                <textarea  cols="30" rows="3" name='mensaje' className={`formGrupo__texto ${error.mensaje && "error"}`} id='mensaje' onBlur={handleBlur} onChange={handleChange} value={form.mensaje} required></textarea>
